...
export interface UserStats {
  ...
  emotionalRange: number; // Number of unique emotions logged
}
...
export function calculateStreak(logs: EmotionData[]): StreakData {
  ...
  if (daysSinceLastLog === 0 || daysSinceLastLog === 1) {
    ...
    // Pause the streak instead of breaking it
    isPaused = true;
    currentStreak = 1; // Keep the last streak count
  }
...
export function getAchievementProgress(achievement: Achievement, stats: UserStats): number {
  switch (achievement.id) {
    case 'first_log':
      return Math.min(stats.totalLogs, 1);
    case 'consistent_logger':
      return Math.min(stats.currentStreak, 7);
    case 'dedicated':
      return Math.min(stats.currentStreak, 30);
    case 'centurion':
      return Math.min(stats.totalLogs, 100);
    case 'breath_master':
      return Math.min(stats.breathingMinutes, 30);
    case 'grounded':
      return Math.min(stats.groundingExercisesCompleted, 5);
    case 'crisis_warrior':
      return Math.min(stats.panicSessionsCompleted, 3);
    case 'emotional_explorer':
      return Math.min(stats.emotionalRange, 10);
    case 'pattern_detective':
      return Math.min(stats.triggersIdentified.size, 5);
    case 'storyteller':
      return Math.min(stats.notesWritten, 20);
    default:
      return 0;
  }
}
...
export function calculateLevel(totalXP: number): { level: number; progress: number; nextLevel: number } {
  const baseXP = 100;
